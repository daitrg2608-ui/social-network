    ...
    files.forEach((img) => {
      ...
      if (img.type.startsWith("video/")) {
        if (images && images.length) {
          setError("Bạn chỉ có thể đăng ảnh hoặc video trong 1 bài viết.");
          return;
        }
        if (
          img.type !== "video/mp4" &&
          ...
          img.type !== "video/x-matroska"
        ) {
          setError(`${img.name} format is unsupported ! only Mp4, Webm, Mov are allowed.`);
          return;
        } else if (img.size > 1024 * 1024 * 50) {
          setError(`${img.name} size is too large max 50mb allowed.`);
          return;
        } else {
          // chỉ cho 1 video để UI đơn giản
          const url = URL.createObjectURL(img);
          setVideos([{ file: img, url }]);
        }
        return;
      }
      if (
            ...
            {videos && videos.length ? (
              <div className="preview1">
                <video
                  src={videos[0].url}
                  controls
                  style={{ width: "100%", maxHeight: "520px", borderRadius: "10px" }}
                />
              </div>
            ) : (
              <div
